...
from middleware import enrolled_required
from model import enroll
from player_char import PlayerChar
from utilities import dice_roll
...
async def create_char(ctx, build_type='normal'):
    if build_type == 'quick':
        confirmation_message = await ctx.send('Are you sure you want to proceed with quick build? You cannot reroll the ability scores if you proceed with it. Type "confirm" within 30 seconds to proceed.')
        try:
            # Wait for the user to confirm within 30 seconds
            user_confirmation = await bot.wait_for('message', timeout=30.0, check=lambda m: m.author == ctx.author and m.content.lower() == 'confirm')
        ...
        await PlayerChar(ctx).roll_build_char(bot)
    else:
